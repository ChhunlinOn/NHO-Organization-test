...
export default function PDFFlipViewer({ filePath }: PDFFlipViewerProps) {
  ...
  const triggerFlipAnimation = (direction: "left" | "right") => {
    if (pageRef.current) {
      ...
      pageRef.current.style.transformOrigin = direction === "right" ? "left" : "right";
      pageRef.current.style.transform = `rotateY(${direction === "right" ? -30 : 30}deg)`;
      pageRef.current.style.opacity = "0.8";

      // Add shadow effect
      pageRef.current.style.boxShadow = direction === "right"
      ...
      setTimeout(() => {
        if (pageRef.current) {
          ...
          pageRef.current.style.boxShadow = "0 4px 8px rgba(0, 0, 0, 0.2)";
        }
      }, 600);
    }
  };
      ...
      <div ref={containerRef} className="w-full max-w-[1000px] relative px-8 sm:px-10">
        <style jsx>{`
          ...
          .flip-page::after {
            ...
            position: absolute;
            top: 0;
            ${currentPage % 2 === 1 ? 'right: 0;' : 'left: 0;'}
            width: 50%;
            height: 100%;
            background: linear-gradient(
              ${currentPage % 2 === 1 ? 'to left' : 'to right'},
              rgba(0, 0, 0, 0.2),
              rgba(0, 0, 0, 0)
            );
            pointer-events: none;
        ...
        {!isError && (
          <div className="relative flex justify-center">
            ...
            <div ref={pageRef} className="flip-page w-full" style={{ transformStyle: "preserve-3d", boxShadow: "0 4px 8px rgba(0, 0, 0, 0.2)" }}>
              <Document
                file={filePath}
                ...
                loading=""
              >
                <Page pageNumber={currentPage} width={pageWidth} className="shadow-none" />
              </Document>
            </div>

...
export default function PDFFlipViewer({ filePath }: PDFFlipViewerProps) {
  ...
  useEffect(() => {
    if (numPages) {
      const newPreloadedPages = [];
      if (currentPage > 1) newPreloadedPages.push(currentPage - 1);
      newPreloadedPages.push(currentPage);
      if (currentPage < numPages) newPreloadedPages.push(currentPage + 1);
  ...
  // Handle click on the page to zoom
  interface PageClickEvent extends React.MouseEvent<HTMLDivElement, MouseEvent> {}

  interface HandlePageClick {
    (event: PageClickEvent): void;
  }

  const handlePageClick: HandlePageClick = useCallback((event) => {
    const rect = event.currentTarget.getBoundingClientRect();
    const x = event.clientX - rect.left;
    const width = rect.width;
    const clickThreshold = width * 0.1; // 10% of the page width
    ...
    if (x < clickThreshold) {
      ...
      // Zoom in or out based on double click
    ...
    <div className="flex flex-col items-center w-full min-h-screen p-4 box-border relative">
      <div className="max-w-4xl w-full bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="flex justify-center items-center mt-4 gap-2">
          ...
          <span>
            Page {currentPage} of {numPages || "?"} (Scale: {(scale * 100).toFixed(0)}%)
          </span>
